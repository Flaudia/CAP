...
1<={timemax(X): time(X), X>0 }<=1.

isLinked(0,1).
isLinked(J1,J2) :- isLinked(J2,J1).

{changeAngleF(J1,J2,A,Ai,T) : joint(J1), joint(J2), J1 > J2, angles(A), hasAngle(J1,Ai,T), A <> Ai, isLinked(J1,J2)} <=1 :- time(T), timemax(X), T < X, T > 0.
...
affectedF(J1,An,Ac,T) :- changeAngleF(J2,_,A,Ap,T), J1>J2, hasAngle(J1,Ac,T), angles(An), An = |Ac + (A-Ap)|\360, time(T).

hasAngle(J1,A,T+1) :- changeAngleF(J1,_,A,_,T).
hasAngle(J1,A,T+1) :- affectedF(J1,A,_,T).
hasAngle(J1,A,T+1) :- hasAngle(J1,A,T), not changeAngleF(J1,_,_,_,T), not affectedF(J1,_,_,T).

:~ timemax(X). [maxspan-X@1,X]
