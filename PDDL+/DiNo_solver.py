...
for k in range(0, len(typologies)):
	...
	for subdir, dirs, files in os.walk(rootdir):	
		...
		if len(folder_name) > 8:
			...
			solutions_directory_path = local_path + "/solutions/DiNo/"+typologies[k]+"/"+folder_name

			times_directory_path = local_path + "/times/DiNo/"+typologies[k]+"/"+folder_name
			...
			for file in files:
				...
				command = "./pyrunlim.py 2> " + times_path + " -t3600 ./DiNo/bin/dino " +  encoding_path + " " + problem_path + " --custom 1 6 5"
				os.system(command)
				...
				command = "mv " + rootdir + "/" + folder_name + "/" + solution_name + " " + local_path + "/solutions/DiNo/" + typologies[k] + "/" + folder_name
				os.system(command)
